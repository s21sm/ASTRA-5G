...
def configuration_update_command_if_part(factor_to_consider, uplink_command, downlink_command, response, possible_response,pcap_file_path,dl_com_invoke_line):

   
    key_estbl_status = factor_to_consider[0]
    message_encoding_type = factor_to_consider[1]
    
    factor_text, security_header_type = configuration_update_command_factor_retrieve_text(uplink_command, downlink_command, key_estbl_status, message_encoding_type,pcap_file_path,dl_com_invoke_line)
    ...
    if response in possible_response:
        ...
        elif (response == 'Configuration update complete' ):
        
            if (key_estbl_status == True and message_encoding_type == 'Protected'  and security_header_type != '0' ):
                status = 'Pass'
                remark = 'Configuration update successfully completed'
                
            elif (key_estbl_status == False):
                status = 'Fail'
                remark = 'Configuration update should not be completed before 5G-AKA completed'
            
            elif (message_encoding_type == 'Plain'):
                status = 'Fail'
                remark = 'Plain Configuration update command  should not be processed'
...
def configuration_update_command_else_part(factor_to_consider, uplink_command, downlink_command,pcap_file_path,dl_com_invoke_line):
