...
def gmm_status_if_part(factor_to_consider, uplink_command, downlink_command, response, possible_response,pcap_file_path,dl_com_invoke_line):
    ...
    if response in possible_response:
        ...
        elif (response == '5GMM status (Message type not compatible with the protocol state)' or '5GMM status (Invalid mandatory information)'):
            status = 'Pass'
...
def gmm_status_else_part(factor_to_consider, uplink_command, downlink_command,pcap_file_path,dl_com_invoke_line):
